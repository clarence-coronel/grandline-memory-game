<template>
  <div class="bg-transparent border-transparent border-2 text-white w-full max-w-[1200px] overflow-auto rounded-lg p-5">
    <form @submit.prevent="save" class="w-full max-w-[400px] mx-auto flex flex-col gap-5">
      <label for="playerName" class="flex flex-col gap-1">
        <span>Name: </span>
        <input @input="toggleSave" v-model.trim="form.playerName" class="focus:outline-none outline-none bg-transparent rounded-sm border-2 border-white p-1" type="text" name="playerName" id="playerName">
      ...
      <div class="flex flex-col gap-1">
        ...
        <div class="flex justify-between gap-5 flex-wrap">
          <label for="difficulty_easy" class="flex items-center justify-start gap-1">
            <span>Easy</span>
            <input @change="handleDifficultyChange" v-model="form.difficulty" type="radio" name="difficulty" id="difficulty_easy" value="EASY">
          </label>
          <label for="difficulty_normal" class="flex items-center justify-start gap-1">
            <span>Normal</span>
            <input @change="handleDifficultyChange" v-model="form.difficulty" type="radio" name="difficulty" id="difficulty_normal" value="NORMAL">
          </label>
          <label for="difficulty_hard" class="flex items-center justify-start gap-1">
            <span>Hard</span>
            <input @change="handleDifficultyChange" v-model="form.difficulty" type="radio" name="difficulty" id="difficulty_hard" value="HARD">
          </label>
          <label for="difficulty_custom" class="flex items-center justify-start gap-1">
            <span>Custom</span>
            <input @change="handleDifficultyChange" v-model="form.difficulty" type="radio" name="difficulty" id="difficulty_custom" value="CUSTOM">
          </label>
        </div>
        <span class="text-orange-500">{{ form.difficulty == "CUSTOM" ? "Custom games are not eligible for leaderboard entry" : "" }}</span>
      </div>

      <label for="numOfCards" class="flex flex-col gap-1">
        <span>Card Count: </span>
        <input @input="toggleSave" v-model="form.numOfCards" :disabled="inputDisabled" class="disabled:bg-neutral-400/70 disabled:text-neutral-300 disabled:hover:cursor-not-allowed focus:outline-none outline-none bg-transparent rounded-sm border-2 border-white p-1" type="text" name="numOfCards" id="numOfCards">
        <span ref="cardWarning" class="text-red-500"></span>
      </label>

      <label for="hp" class="flex flex-col gap-1">
        <span>HP: </span>
        <input @input="toggleSave" v-model="form.hp" :disabled="inputDisabled" class="disabled:bg-neutral-400/70 disabled:text-neutral-300 disabled:hover:cursor-not-allowed focus:outline-none outline-none bg-transparent rounded-sm border-2 border-white p-1" type="text" name="hp" id="hp">
        <span ref="hpWarning" class="text-red-500"></span>
      </label>

      <label for="damage" class="flex flex-col gap-1">
        <span>Damage: </span>
        <input @input="toggleSave" v-model="form.damage" :disabled="inputDisabled" class="disabled:bg-neutral-400/70 disabled:text-neutral-300 disabled:hover:cursor-not-allowed focus:outline-none outline-none bg-transparent rounded-sm border-2 border-white p-1" type="text" name="damage" id="damage">
        <span ref="damageWarning" class="text-red-500"></span>
      </label>

      <button :disabled="saveDisabled" class="mt-10 disabled:bg-neutral-400 bg-green-600 hover:bg-green-500 duration-200 font-bold text-white rounded-sm p-2">
        Save
      </button>
    </form>
  </div>
