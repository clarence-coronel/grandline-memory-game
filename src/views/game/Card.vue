<template>
    <button @click="toggleShowCard" :class="{'show-card': showCard, 'hide-card hover:shadow-secondary ': !showCard}" class="card-bg focus:outline-none outline-none flex justify-center p-1 md:p-3 items-center duration-500 w-full max-w-48 aspect-[5/8] rounded-lg bg-primary">
        <div class="w-full h-full flex flex-col justify-center items-center rounded-lg overflow-hidden">
            ...
            <div :class="{'hidden': showContent, 'flex': !showContent}" class="w-full h-full flex-col justify-center items-center p-3 md:p-5">
                <span class="text-white text-lg font-bold md:hidden">{{index}}</span>
